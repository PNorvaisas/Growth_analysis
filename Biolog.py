...
for mod in ['pip','string','math','re','csv','sys','os',
            ...
            'types','math','copy','pyExcelerator','xlrd','xlwt','xlutils','types','warnings','unicodedata']:
    try:
...
def readinfo(ifile):
    ...
    if itp in ['xlsx','xls']:
        ...
        raise Exception("Unknown design file format!")
    ...
    if not all(n in headers for n in nec):
        print [not n in headers for n in nec]
...
def plot_comparison(data,metabolites,dirn,figs,info,uniques):
    ...
    for uk in sorted(uniques.keys()):
        ...
        for fg in figures:
            ...
            plot=plot_2D(ttl,fg,bplate,data,cgroup,egroup,metsel,info)
